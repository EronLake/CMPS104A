...
int main (int argc, char** argv) {
  ...
  //    printf("------------------------------------------------\n");
      ...
      print_symtable (sym_output_file, parser::root, 0);
   }
   ...
   //---------------------------------------------------------
   //creates the output file name with the ".sym" extension
   const char* oil_output_name = mk_outname(filename ,".oil");

   //dumps the output to the file output.str
   FILE* oil_output_file = fopen (oil_output_name, "w");
   if(oil_output_file == NULL){perror("Error opening file");}

   //traversal funcitons
   if (parser::root != NULL){
  //    printf("------------------------------------------------\n");
      print_oil (oil_output_file, parser::root, 0);
   }
   fclose(oil_output_file);
